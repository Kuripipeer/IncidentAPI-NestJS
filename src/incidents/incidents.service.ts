...
const CACHE_KEY_ALL_INCIDENTS = "incidentes:all";
...
export class IncidentsService {
    ...
    async findByRadius(lat:number, lon:number, radius:number) : Promise<Incident[]>{
        try {
            // POSTGIS
            // SQL
    ...
    async findAll() : Promise<Incident[]>{
        try {
            ...
            console.error("[IncidentService] ocurrio un error al querer traer los incidentes")
            console.error(error);
            return [];
        }
    }
    ...
    async createIncident(incident:IncidentCDto): Promise<Boolean>{
        // save
        // Generar un nuevo registro de la entidad incident!!!
